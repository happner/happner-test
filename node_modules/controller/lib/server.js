...
ControllerServer.prototype.createMinion = function($happn, registration, callback) {
  ...
  $happn.log.info('createMinion \'%s\'', minion.name);
...
ControllerServer.prototype.destroyMinion = function($happn, registration, callback) {
  ...
  $happn.log.info('destroyMinion \'%s\'', minion.name);

  if (marshal.minions[minion.name].status == 'error' || marshal.minions[minion.name].status == 'done') {
    setTimeout(function() {
...
ControllerServer.prototype.updateMinion = function($happn, update, callback) {
  ...
  $happn.log.info('updateMinion \'%s\'', minion.name);

  if (!marshal.minions[minion.name]) {
    delete marshal.minions[minion.id]
  }

  marshal.minions[minion.name] = minion;
  callback();
