...
ControllerServer.prototype.registerMinion = function($happn, registration, callback) {

  var name = registration.info.mesh.name;
  var marshal = this.marshals[name];
  var minion = registration.minion;

  $happn.log.info('register minion \'%s\'', minion.name);

  marshal.minions[minion.name] = minion.data;
  callback();

}


ControllerServer.prototype.deRegisterMinion = function($happn, registration, callback) {

  var name = registration.info.mesh.name;
  var marshal = this.marshals[name];
  var minion = registration.minion;

  $happn.log.info('de-register minion \'%s\'', minion.name);

  delete marshal.minions[minion.name];
  callback();
